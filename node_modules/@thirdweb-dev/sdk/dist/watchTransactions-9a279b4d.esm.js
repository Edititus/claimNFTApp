import { b3 as getSignerAndProvider, aM as resolveAddress, al as getCompositeABI, bj as AbiSchema, j as getContractTypeForRemoteName, h as PREBUILT_CONTRACTS_MAP } from './index-408caea6.esm.js';
...
async function getContractFromAbi(params) {
  ...
  const [resolvedAddress, {
    ...
    SmartContract
  }] = await Promise.all([resolveAddress(params.address), provider.getNetwork(), import('./smart-contract-76db1ed0.esm.js')]);
...
async function getContract(params) {
  ...
  const [resolvedAddress, {
    ...
    ContractPublisher
  }] = await Promise.all([resolveAddress(params.address), provider.getNetwork(), import('./contract-publisher-7b7060e4.esm.js').then(function (n) { return n.c; })]);
  if (inContractCache(resolvedAddress, chainId)) {
    return getCachedContract(resolvedAddress, chainId);
  }
