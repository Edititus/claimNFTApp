 ...
 */
export const EmailSelectionUI = ({ onSelect, walletConfig, auth }) => {
    const l = useLocale();
    ...
    useEffect(() => {
        const emailWalletInstance = createWalletInstance(walletConfig);
    ...
    return (<Box paddingHorizontal="xl" mt="lg">
      {isGoogleEnabled ? (<Box justifyContent="center">
          <WalletButton iconHeight={28} iconWidth={28} borderRadius="lg" borderWidth={1} borderColor="buttonBackgroundColor" backgroundColor="buttonBackgroundColor" nameColor="buttonTextColor" justifyContent="center" name={l.embedded_wallet.sign_in_google} walletIconUrl={GOOGLE_ICON} onPress={onGoogleSignInPress}/>
          {isEmailEnabled ? (<Box mb="md" mt="md" flexDirection="row" justifyContent="center" alignItems="center">
      ...
      {isEmailEnabled ? (<>
          ...
          <BaseButton mt="md" paddingVertical="md" borderRadius="lg" borderWidth={1} borderColor="border" backgroundColor="accentButtonColor" onPress={handleNetworkCall}>
            {isFetching ? (<ActivityIndicator size={"small"} color={theme.colors.accentButtonTextColor}/>) : (<Text variant="bodySmallBold" color="accentButtonTextColor">
                {l.common.continue}
              </Text>)}
          </BaseButton>
        </>) : null}
      {errorMessage ? (<Text variant="error" mt="xxs">
          {errorMessage}
        </Text>) : null}
    </Box>);
};
