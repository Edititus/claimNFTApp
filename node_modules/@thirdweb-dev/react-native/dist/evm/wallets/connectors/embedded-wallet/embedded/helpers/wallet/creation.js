...
import { AUTH_SHARE_INDEX, DEVICE_SHARE_INDEX, DEVICE_SHARE_MISSING_MESSAGE, RECOVERY_SHARE_INDEX, } from "../constants";
import { setDeviceShare } from "../storage/local";
import { encryptShareWeb } from "./encryption";
export async function setUpNewUserWallet(recoveryCode, clientId) {
    try {
...
// Wallet Creation
async function createEmbeddedWallet({ clientId, recoveryCode, }) {
...
function createWalletShares() {
    ...
    const privateKeyHex = Buffer.from(`thirdweb_${wallet.privateKey}`).toString("hex");
 ...
 */
export async function storeShares({ clientId, walletAddress, authShare, deviceShare, recoveryShares, }) {
    ...
    try {
        ...
        throw new Error(`Malformed response from the ews store user share API: ${JSON.stringify(e)}`);
    }
}
