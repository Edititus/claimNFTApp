...
import { generateAuthTokenFromCognitoEmailOtp, getEmbeddedWalletUserDetail, sendUserManagedEmailOtp, validateUserManagedEmailOtp, verifyClientId, } from "./helpers/api/fetchers";
import { cognitoEmailSignIn, cognitoEmailSignUp, } from "./helpers/auth/cognitoAuth";
import { postPaperAuth, postPaperAuthUserManaged, prePaperAuth, } from "./helpers/auth/middleware";
...
import { ROUTE_AUTH_JWT_CALLBACK, ROUTE_HEADLESS_GOOGLE_LOGIN, } from "./helpers/constants";
...
export async function sendVerificationEmail(options) {
    ...
    try {
        ...
        throw new Error(`Malformed response from the send email OTP API: ${JSON.stringify(e)}`);
    }
    ...
    return result.isNewUser
        ...
        : {
            isNewUser: result.isNewUser,
            isNewDevice: !(await isDeviceSharePresentForUser(options.clientId, result.walletUserId ?? "")),
            recoveryShareManagement: result.recoveryShareManagement,
        };
}
export async function validateEmailOTP(options) {
    ...
    if (result.recoveryShareManagement === "USER_MANAGED") {
        ...
        try {
            ...
            verifiedTokenResponse = await generateAuthTokenFromCognitoEmailOtp(session, options.clientId);
        }
        catch (e) {
    ...
    try {
        ...
        throw new Error(`Malformed response from the verify one time password: ${e.message}}`);
    }
}
export async function socialLogin(oauthOptions, clientId) {
    const headlessLoginLinkWithParams = `${ROUTE_HEADLESS_GOOGLE_LOGIN}?authProvider=${encodeURIComponent(oauthOptions.provider)}&baseUrl=${encodeURIComponent("https://embedded-wallet.thirdweb.com")}&platform=${encodeURIComponent("mobile")}`;
    ...
    const completeLoginUrl = `${platformLoginLink}?developerClientId=${encodeURIComponent(clientId || "")}&platform=${encodeURIComponent("mobile")}&redirectUrl=${encodeURIComponent(oauthOptions.redirectUrl)}`;
    const result = await InAppBrowser.openAuth(completeLoginUrl, oauthOptions.redirectUrl, {
        // iOS Properties
    ...
    try {
        ...
        throw new Error(`Malformed response from post authentication: ${JSON.stringify(e)}`);
    }
}
export async function customJwt(authOptions, clientId) {
