...
export async function getEncryptionKey(pwd, salt, iterationCounts) {
    const key = QuickCrypto.pbkdf2Sync(pwd, salt, iterationCounts, KEY_LENGTH, "sha256");
...
export async function encryptShareWeb(share, pwd) {
    ...
    const returnValue = `${bufferToBase64(cipherTextWithTag)}${ENCRYPTION_SEPARATOR}${ivBase64}${ENCRYPTION_SEPARATOR}${bufferToBase64(salt)}${ENCRYPTION_SEPARATOR}${iterationCount}`;
    return returnValue;
}
export async function decryptShareWeb(encryptedShareDetails, pwd) {
    ...
    const key = await getEncryptionKey(pwd, base64ToBuffer(saltBase64), iterationCount);
    const encryptedShareWithTagBuffer = Buffer.from(encryptedShareWithTagBase64, "base64");
    ...
    const cipherTextBuffer = Buffer.from(encryptedShareWithTagBuffer.subarray(0, cipherTextBufferLength));
    const tagBuffer = Buffer.from(encryptedShareWithTagBuffer.subarray(cipherTextBufferLength));
    ...
    const normalizedShare = await AesGcmCrypto.decrypt(originalBase64CipherText, key, ivBufferHex, hexStringTag, false);
