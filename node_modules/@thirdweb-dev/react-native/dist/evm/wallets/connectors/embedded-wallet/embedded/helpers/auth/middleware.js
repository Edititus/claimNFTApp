import { AuthProvider, RecoveryShareManagement, } from "@thirdweb-dev/wallets";
import { getDeviceShare, setAuthTokenClient, setWallerUserDetails, } from "../storage/local";
...
export async function postPaperAuth({ storedToken, clientId, recoveryCode, }) {
    ...
    if (storedToken.isNewUser) {
        const _recoveryCode = await getRecoveryCode(storedToken, clientId, recoveryCode);
        if (!_recoveryCode) {
...
export async function postPaperAuthUserManaged(storedToken, clientId, password) {
...
async function getRecoveryCode(storedToken, clientId, recoveryCode) {
...
function extractSubFromJwt(jwtToken) {
