...
const ImageSvgUri = ({ imageUrl = "", width, height, imageAlt = "", }) => {
    ...
    return (<Box width={width} height={height}>
      {error ? (<SvgUri width={width} height={height} uri={resolvedImageUrl}/>) : (
        // always try to render Image first, if error then try to render svg
        // Image from RN handles onError better than SvgUri
        <Image alt={imageAlt} source={{ uri: resolvedImageUrl }} style={[{ width: width, height: height }]} onError={() => setError(true)}/>)}
    </Box>);
};
