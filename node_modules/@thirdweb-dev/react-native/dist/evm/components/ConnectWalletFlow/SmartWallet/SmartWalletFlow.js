...
import { useConnect, useCreateWalletInstance, useWalletContext, } from "@thirdweb-dev/react-core";
...
export const SmartWalletFlow = ({ connected, goBack, walletConfig, personalWalletConfig, ...props }) => {
    const l = useLocale();
    const theme = useGlobalTheme();
    const [connectedPersonalWallet, setConnectedPersonalWallet] = useState();
    ...
    const connectSmartWallet = useCallback(async (personalWallet) => {
    ...
    const connectPersonalWallet = useCallback(async (wallet) => {
    ...
    const onPersonalWalletConnected = useCallback((walletInstance) => {
        setConnectedPersonalWallet(walletInstance);
        connectSmartWallet(walletInstance);
    }, [connectSmartWallet]);
    return (<>
      {PersonalWalletConfigUI && !connectedPersonalWallet ? (<PersonalWalletConfigUI {...props} connected={connected} goBack={goBack} walletConfig={personalWalletConfig} onLocallyConnected={onPersonalWalletConnected}/>) : (<Box paddingHorizontal="xl">
          <ConnectWalletHeader subHeaderText="" onBackPress={onConnectingBackPress} onClose={onConnectingClosePress}/>
          <WalletLoadingThumbnail imageSize={85}>
            <ImageSvgUri height={80} width={80} imageUrl={walletConfig.meta.iconURL}/>
          ...
          <View style={styles.connectingContainer}>
            <>
              ...
              {mismatch ? (<Text variant="bodySmallSecondary" mt="lg" textAlign="center">
      ...
      {mismatch === true ? (<BaseButton marginVertical="sm" mt="md" marginHorizontal="xl" flexDirection="row" justifyContent="center" alignItems="center" paddingHorizontal="md" paddingVertical="sm" borderRadius="sm" backgroundColor="backgroundHighlight" onPress={onSwitchNetworkPress}>
          {switchingNetwork ? (<ActivityIndicator size="small" color={theme.colors.textPrimary}/>) : (<Text variant="bodyLarge" textAlign="center">
