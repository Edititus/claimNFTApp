...
import { useChain, useDisconnect, useSetConnectedWallet, useWallet, } from "@thirdweb-dev/react-core";
...
export const ConnectWalletDetailsModal = ({ isVisible, onClosePress, extraRows, address, hideTestnetFaucet, supportedTokens, displayBalanceToken, }) => {
    const l = useLocale();
    ...
    const onWalletImported = useCallback((localWalletInstance) => {
    ...
    const getAdditionalActions = useCallback(() => {
        if (activeWallet?.walletId === SmartWallet.id || smartWallet) {
            return (<SmartWalletAdditionalActions onExportPress={onExportLocalWalletPress}/>);
        }
        if (activeWallet?.walletId === walletIds.localWallet) {
          ...
          <BaseButton backgroundColor="background" borderColor="border" borderRadius="lg" borderWidth={0.5} mb="sm" justifyContent="space-between" style={styles.exportWallet} onPress={onExportLocalWalletPress}>
            <>
          ...
          <BaseButton backgroundColor="background" borderColor="border" borderRadius="lg" borderWidth={0.5} mb="sm" justifyContent="space-between" style={styles.exportWallet} onPress={onImportLocalWalletPress}>
            <>
              <PocketWalletIcon width={16} height={16}/>
              <View style={styles.exportWalletInfo}>
                <Text variant="bodySmall">
          ...
          <LocalWalletImportModal isVisible={isImportModalVisible} onWalletImported={onWalletImported} onClose={onImportModalClose}/>

          {activeWallet?.walletId === LocalWallet.id ? (<Text variant="error" textAlign="left" mb="sm">
      ...
      <TWModal isVisible={isVisible} onBackdropPress={onClosePress}>
        <Box backgroundColor="background" style={styles.modal}>
          <Box style={styles.contentContainer}>
            <ExportLocalWalletModal isVisible={isExportModalVisible} onClose={onExportModalClose}/>
            <WalletDetailsModalHeader tokenAddress={tokenAddress} address={address} onDisconnectPress={onDisconnectPress} onAddressCopied={onAddressCopied}/>
            {activeWallet?.walletId === SmartWallet.id ? (<BaseButton disabled={!isSmartWalletDeployed} onPress={() => {
                Linking.openURL(`https://thirdweb.com/${chain?.slug}/${address}/account`);
            }} flexDirection="row" alignItems="center" justifyContent="space-between" mt="md">
                ...
                {isSmartWalletDeployed ? (<RightArrowIcon width={10} height={10}/>) : null}
              </BaseButton>) : null}
            {activeWallet?.walletId === EmbeddedWallet.id &&
            activeWallet.getEmail() ? (<Box flexDirection="row" alignItems="center" mt="md">
                <ActiveDot width={10} height={10}/>
                <Text variant="bodySmallSecondary" ml="xxs">
                  {activeWallet.getEmail()}
                </Text>
              </Box>) : null}
            <Box flexDirection="row" justifyContent="space-evenly" marginVertical="md">
            ...
            {!hideTestnetFaucet && chain?.testnet && chain?.faucets?.length ? (<IconTextButton mt="xs" text={l.connect_wallet_details.request_testnet_funds} icon={<MoneyIcon height={16} width={16}/>} onPress={() => {
                if (chain?.faucets?.[0]) {
                    Linking.openURL(chain.faucets[0]);
                }
            }}/>) : null}
            {chain?.explorers && chain?.explorers?.[0] && (<IconTextButton mt="xs" text={l.connect_wallet_details.view_transaction_history} icon={<TransactionIcon height={16} width={16}/>} onPress={() => {
                Linking.openURL(chain?.explorers?.[0].url + "/address/" + address);
            }}/>)}
            {getAdditionalActions()}
            {extraRows ? extraRows({}) : null}
            {addressCopied === true ? (<Toast text={"Address copied to clipboard"}/>) : null}
          </Box>
        </Box>
      </TWModal>
    </ThemeProvider>);
