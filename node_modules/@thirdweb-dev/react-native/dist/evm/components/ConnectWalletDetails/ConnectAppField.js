...
const ConnectAppField = () => {
    ...
    const onSmartWalletWCMessage = useCallback(({ type }) => {
    ...
    const onAddressChangeText = (text) => {
        setWCUri(text);
    };
    ...
    return (<>
      {!appMeta && showWCInput ? (<Box flexDirection="row" mt="xs" borderColor="border" borderWidth={1} borderRadius="md">
          ...
          <BaseButton onPress={() => onWCPress()} justifyContent="center" alignItems="center">
            <RightArrowIcon height={20} width={30} color={theme.colors.iconPrimary}/>
          </BaseButton>
          <BaseButton onPress={onScanQRPress} justifyContent="center" alignItems="center" marginHorizontal="xs">
            <QrCodeIcon height={30} width={30} color={theme.colors.iconPrimary}/>
          </BaseButton>
        </Box>) : (<BaseButton backgroundColor="background" borderColor="border" mt="xs" justifyContent="space-between" style={styles.exportWallet} onPress={onConnectDappPress}>
          <>
            {appMeta?.iconUrl ? (<WalletIcon size={24} iconUri={appMeta.iconUrl}/>) : (<WalletConnectIcon width={24} height={24}/>)}
          ...
          {appMeta ? (<DisconnectIcon height={10} width={10} color={theme.colors.iconPrimary}/>) : (<RightArrowIcon height={10} width={10} color={theme.colors.iconPrimary}/>)}
        </BaseButton>)}

      <QRCodeScan isVisible={showQRCodeScan} onQRCodeScan={onQRCodeScan} onClose={onQRClose}/>
    </>);
};
