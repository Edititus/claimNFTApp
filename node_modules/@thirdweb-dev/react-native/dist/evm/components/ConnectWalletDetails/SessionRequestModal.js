...
export const SessionRequestModal = () => {
    ...
    const { data: requestData } = modalState;
    ...
    const getContent = useCallback(() => {
        switch (requestData.method) {
            case EIP155_SIGNING_METHODS.SWITCH_CHAIN:
                return (<Text variant="bodySmall" textAlign="left">
            {`${l.connect_wallet_details.switch_to} ${requestData.params[0].chainId === chain?.chainId
                        ? chain?.slug
    ...
    return (<Box flexDirection="column" backgroundColor="background" borderRadius="md" p="lg">
      ...
      <Box flexDirection="row" justifyContent="space-evenly" mt="lg">
        <BaseButton alignContent="center" alignItems="center" borderRadius="sm" borderWidth={0.5} disabled={approvingRequest} paddingHorizontal="sm" paddingVertical="md" minWidth={100} borderColor="border" onPress={async () => {
            wallet.rejectRequest();
            onClose();
        }}>
          <Text variant="bodySmall">{l.common.reject}</Text>
        </BaseButton>
        <BaseButton alignContent="center" alignItems="center" borderRadius="sm" borderWidth={0.5} disabled={approvingRequest} paddingHorizontal="sm" paddingVertical="md" minWidth={100} backgroundColor="white" borderColor="border" onPress={onApprove}>
          {approvingRequest ? (<ActivityIndicator size="small"/>) : (<Text variant="bodySmall" color="black">
              {l.common.approve}
            </Text>)}
        </BaseButton>
      </Box>
