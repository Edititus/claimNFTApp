...
export const SmartWalletAdditionalActions = ({ onExportPress, }) => {
    const l = useLocale();
    ...
    const wallet = showSmartWallet
        ? smartWallet
        : activeWallet?.getPersonalWallet();
    useEffect(() => {
    ...
    return (<>
      <IconTextButton mt="xs" text={showSmartWallet
            ? l.smart_wallet.switch_to_smart
            : l.smart_wallet.switch_to_personal} icon={<DisconnectIcon width={14} height={14} color={theme.colors.iconPrimary}/>} onPress={onWalletPress}/>
      {!showSmartWallet && smartWallet?.enableConnectApp ? (<ConnectAppField />) : null}
      {wallet?.walletId === walletIds.localWallet ||
            activeWallet?.walletId === walletIds.localWallet ? (<>
          <BaseButton backgroundColor="background" borderColor="border" borderRadius="lg" borderWidth={0.5} mb="sm" mt="xs" justifyContent="space-between" style={styles.exportWallet} onPress={onExportPress}>
            <>
            ...
            <RightArrowIcon height={10} width={10} color={theme.colors.iconPrimary}/>
          </BaseButton>
