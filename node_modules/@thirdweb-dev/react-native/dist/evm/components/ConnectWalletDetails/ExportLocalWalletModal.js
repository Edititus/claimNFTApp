...
import { ActivityIndicator, KeyboardAvoidingView, Platform, Share, StyleSheet, } from "react-native";
...
export const ExportLocalWalletModal = ({ isVisible, onClose, }) => {
    const l = useLocale();
    ...
    const onContinuePress = async () => {
        ...
        if (activeWallet?.walletId === SmartWallet.id) {
            data = await activeWallet.getPersonalWallet().export({
                strategy: "encryptedJson",
                password: password,
            });
        }
        else {
            data = await activeWallet.export({
                strategy: "encryptedJson",
                password: password,
            });
        }
        ...
        if (Platform.OS === "android") {
            ...
            if (permissions.granted) {
                ...
                try {
                    ...
                    fileURI = await FileSystem.StorageAccessFramework.createFileAsync(uri, fileName, "application/json");
                }
                catch (e) {
    ...
    return (<TWModal isVisible={isVisible} backdropOpacity={0.7}>
      <KeyboardAvoidingView behavior="padding">
        <Box flexDirection="column" backgroundColor="background" borderRadius="md" p="lg">
          <ModalHeaderTextClose onClose={onCloseInternal} headerText={l.local_wallet.backup_your_wallet}/>
          ...
          <Text variant="bodySmallSecondary">
            {shortenWalletAddress(personalWalletAddress ? personalWalletAddress : address)}
          </Text>
          <Text variant="bodySmall" textAlign="left" mt="lg" mb="xxs">
            {l.common.password}
          </Text>
          ...
          <Box flexDirection="row" justifyContent="flex-end" mt="lg">
            <BaseButton backgroundColor="white" style={styles.modalButton} onPress={onContinuePress}>
              {isExporting ? (<ActivityIndicator size="small" color="buttonTextColor"/>) : (<Text variant="bodySmall" color="black">
